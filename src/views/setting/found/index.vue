<template>
    <table-modal :tableHeader="column" tableUrl="appDiscovery.list" ref="tableRef" :form-config="formConfig" :valueForm="queryParam">
      <template #default="scope">
        <slot v-if="scope.row && scope.row.key == 'Tag_Profile_IM_Gender'">
          <div>{{ genderList[scope.row.Tag_Profile_IM_Gender] }}</div>
        </slot>

        <slot v-if="scope.row && scope.row.key == 'type'">
          <el-tag round type="info">{{ scope.row.type }}</el-tag>
        </slot>

        <slot v-if="scope.row && scope.row.key == 'icon'">
            <el-image v-if="scope.row.icon" style="width: 100px; height: 100px" :src="scope.row.icon" fit="scale-down" />
        </slot>

        <slot v-if="scope.row && scope.row.key == 'icon_pc'">
            <el-image v-if="scope.row.icon_pc" style="width: 100px; height: 100px" :src="scope.row.icon_pc" fit="scale-down" />
        </slot>
  
        <slot v-if="scope.row && scope.row.key == 'status'">
          <el-tag round :type="scope.row.status==1?'success':'danger'">{{ statusList[scope.row.status] }}</el-tag>
        </slot>
    ...
    <add-modal v-if="isAddShow" :addVisible="isAddShow" :tagData="state.tagData" @updateList="$upTableList(tableRef)"
  ...
  const column = [
    ...
    { title: "排序", key: "order" },
    { title: "分组类型", key: "type", isNeedSlot: true },
    { title: "名称", key: "name" },
    { title: "图标", key: "icon", isNeedSlot: true, width: 130 },
    { title: "pc端图标", key: "icon_pc", isNeedSlot: true, width: 130 },
    { title: "链接", key: "link" },
    { title: "创建时间", key: "create_at", keyType: 'time', width: 90 },
    { title: "状态", key: "status", isNeedSlot: true },
    { title: "操作", key: "edit_row", width: 90 },
  ];
  ...
  const getChannelList = () => {
    $api.channel
      ...
      .then((res) => {
        ...
        formConfig.formItemList[0].options = data;
  ...
  const getTag = () => {
    $api.dictionary
      ...
      .then((res) => {
        // console.log("res", res);
        let a = res.data.list.find((item) => item.name == "Icon_Type");
