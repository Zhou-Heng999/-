...
watch(
  ...
  (val) => {
    ...
    if (val.percent_json) {
      let objList = JSON.parse(val.percent_json);
      objList.forEach((item) => {
        item.second = item.second / 3600;
      });
      editableTabs.value = objList;

      console.log(111, objList);
    }
    form.open_time = form.open_time * 1000;
  },
  {
    immediate: true,
  }
...
const uploadAd = (content) => {
  ...
  $api.parameter
    ...
    .then((res) => {
      console.log(res);
      form.imageUrl = URL.createObjectURL(content.file);
