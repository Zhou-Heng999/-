<template>
    <el-dialog v-model="props.addVisible" width="80vw" @close="emit('closeModal')" title="黑名单">
        <div v-loading="loading">
            <table-modal v-if="state.isShowTable" :tableHeader="column" :valueForm="queryParam"
                tableUrl="appUserBlack.list" ref="tableRef" height="50vh">
                <template #default="scope">
                    <slot v-if="scope.row && scope.row.key == 'edit_row'">
                        
                        <el-popconfirm confirm-button-text="确定" cancel-button-text="取消" icon="el-icon-info" icon-color="red"
                            title="确定移除黑名单吗？" @confirm="handleDelete(scope.$index, scope.row)">
                            <template #reference>
                                <el-button size="small" type="danger">移除黑名单</el-button>
                            </template>
                        </el-popconfirm>
                    </slot>
