    ...
    <el-dialog v-model="props.addVisible" width="80vw" @close="emit('closeModal')" title="群员列表">
        <div v-loading="loading">
            <table-modal v-if="state.isShowTable" :tableHeader="column" :valueForm="queryParam"
                :tableList="state.excelList" ref="tableRef" height="49vh">
                ...
                <template #leftView>
                    <el-button style="margin-right: 20px;" type="primary" @click="downloadTemplate">下载模板</el-button>
                    <el-upload
                    ...
                    <div style="margin-left: 20px;color: gray;font-size: 14px;">请在下载的模板中依次填入账号,昵称,性别,个性签名,地址,生日,群id</div>
                </template>
                <template #rightView>
                    <el-button style="margin-right: 20px;" type="success" @click="startImport">开始导入</el-button>
                </template>
            </table-modal>
        </div>
    </el-dialog>
...
const downloadTemplate=() =>{
    ...
    a.href = "./static/template.xlsx";  //对应模板
...
const startImport = () => {
    ...
    newArr.forEach(item=>{
        item.gender = genderType[item.gender]
        item.account = (item.account).toString()
