<template>
  <div class="table-container">
    ...
    <div class="button-group">
      <el-button type="primary" :icon="Plus" @click="isAddShow = true">新建提现商户</el-button>
    </div>
    <el-table
            ref="filterTableRef"
            stripe
    ...
    >
      ...
      <el-table-column prop="isOpen" label="状态">
        <template #default="scope">
          <el-button size="small" :type="scope.row.isOpen == true ? 'success' : 'warning'">
...
<script setup>
  ...
  const formConfig = reactive({
    // 搜索栏配置项
    formItemList: [
      { type: "input", placeholder: "输入商户名称", key: "name" },
    ],
  });
  ...
  onMounted(() => {
    ...
    getbuyUserList(param);
  });

  // methods
  // 获取表格数据
  const getbuyUserList = (param) => {
    $api.trans
            .buy_user_list(param)
            .then((res) => {
              state.tableData = res.data;
              console.log("getbuyUserList", res);
