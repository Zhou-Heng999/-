<template>
    <el-dialog v-model="props.addVisible" width="632px" @close="emit('closeModal')" title="批量发私聊消息">
        <div v-loading="loading" class="new">
            <el-form ref="formRef" :model="form" :rules="rules" label-position="right" label-width="100px">
                ...
                <el-form-item label="接收人群" prop="toAccount">
                    <el-input v-model="form.toAccount" type="textarea" placeholder="请输入接收账号，多个用','隔开" />
                </el-form-item>
...
<!-- 
                <el-form-item label="消息保存时长" prop="msgLifeTime">
                    <el-input v-model="form.msgLifeTime" type="number" placeholder="最长为7天（604800秒），0则消息只发在线用户" />
                </el-form-item> -->
        ...
        if (valid) {
            ...
            if(form.msgtype=='TIMTextElem'){
                params = {
                    ...
                    toAccount: form.toAccount.split(","),
                }
            }

            editLoading.value = true;
            $api.singleMsg
                .batch_send_msg(params)
                .then((res) => {
